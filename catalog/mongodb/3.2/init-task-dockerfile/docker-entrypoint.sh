...
if [ -z "$REGION" ] || [ -z "$CLUSTER" ] || [ -z "$MANAGE_SERVER_URL" ] || [ -z "$SERVICE_NAME" ] || [ -z "$SERVICE_MASTER" ] || [ -z "$REPLICA_SET_NAME" ]
then
...
output=$(mongo $SERVICE_MASTER --eval "JSON.stringify(db.adminCommand({'replSetInitiate' : $cfg}))")
echo $output
...
curl -X PUT -H "Content-Type: application/json" -d $data $MANAGE_SERVER_URL"/?SetServiceInitialized"
